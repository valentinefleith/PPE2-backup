...
def iterate_pathlib(folder: str | Path, filtered_sources: Optional[list[str]]) -> list[Path]:
    """
...
def read_corpus(
    folder : str | Path,
    sources : list[str],
    iterator : Callable[[list[str | Path]], list[Path]],
    reader : Callable[[str | Path], list[Item]],
